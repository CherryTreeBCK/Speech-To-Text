...
class GUI:
    def __init__(self, width=900, height=50):
        ...
        self.text_label = tk.Label(self.root, text="Your text will appear here",
                                   fg=self.default_font_color, bg="white", anchor="w")
        self.text_label.place(relx=0, rely=0.5, anchor="w")
    ...
    def toggle_translate(self):
        config.TRANSLATE = not config.TRANSLATE
        print(f"TRANSLATE is now: {config.TRANSLATE}")

    def change_translation_language(self):
        import config
        new_lang = simpledialog.askstring("Set Language", "Enter target language code (e.g., en, ko, ja, fr):", initialvalue=config.LANGUAGE)
        if new_lang:
            config.LANGUAGE = new_lang.strip().lower()
